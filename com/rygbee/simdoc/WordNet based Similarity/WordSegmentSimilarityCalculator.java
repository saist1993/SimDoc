...
public class WordSegmentSimilarityCalculator extends SimilarityCalculator
{		
	private String _segment1;
	private String _segment2;
	private String[] _segment1tokenlist;
	private String[] _segment2tokenlist;
	private double[][] _SmithWaterman_word_matrix;
	...
	public WordSegmentSimilarityCalculator(String similaritymeasure, String simtype) 
	{		
	...
	public void resetSegmentSimilarityComputer()
	{
		...
		_editpenalizer.setLValue(0);
	}
	
	//NOTE: wordsegment represents one sentence (in each document) modeled as one word-sequence
	
	public double gmentSimilarity(String wordsegment1, String wordsegment2)
	{
		_segment1 = wordsegment1;
		_segment2 = wordsegment2;
        ...
        _SmithWaterman_word_matrix = new double[_segment1tokenlist.length + 1][_segment2tokenlist.length + 1]; //NOTE: i-th position of _SmithWatermanmatrix corresponds to (i-1)th position in segment*/
			...
			for (int segment2tokenindexinmatrix = 0; segment2tokenindexinmatrix <= _segment2tokenlist.length; segment2tokenindexinmatrix++) 
			{
				if (segment1tokenindexinmatrix != 0 && segment2tokenindexinmatrix != 0)
                {
                	...
                	if (_segment1tokenlist[segment1tokenindexinmatrix - 1].equals(_segment2tokenlist[segment2tokenindexinmatrix - 1])) 
                    {
                    	...
                    	if(_SmithWaterman_word_matrix[segment1tokenindexinmatrix - 1][segment2tokenindexinmatrix - 1] == 0 && _SmithWaterman_word_matrix[segment1tokenindexinmatrix - 1][segment2tokenindexinmatrix] == 0 && _SmithWaterman_word_matrix[segment1tokenindexinmatrix][segment2tokenindexinmatrix - 1] == 0)
                        {
                    		//_editpenalizer.setWords(_segment1tokenlist[segment1tokenindexinmatrix - 1], _segment2tokenlist[segment2tokenindexinmatrix - 1]);
                    		_SmithWaterman_word_matrix[segment1tokenindexinmatrix][segment2tokenindexinmatrix] = 1;
                    	...
                    	else
                    	{
	                		...
	                		_SmithWaterman_word_matrix[segment1tokenindexinmatrix][segment2tokenindexinmatrix] = Math.max(0, _SmithWaterman_word_matrix[segment1tokenindexinmatrix - 1][segment2tokenindexinmatrix - 1] + 1);
	                		
	                		_globalmatrixmatchscore = _SmithWaterman_word_matrix[segment1tokenindexinmatrix][segment2tokenindexinmatrix] + _globalmatrixmatchscore;
	                		_upperboundcumulativescore = _SmithWaterman_word_matrix[segment1tokenindexinmatrix][segment2tokenindexinmatrix] + _upperboundcumulativescore;
	                		//_upperboundcumulativescore++;
	                		_editpenalizer.setLValue(0); // A semantic match should reset the l-value
	                		_lastbestglobalmatrixmatchscore = _globalmatrixmatchscore; 
                    ...
                    else 
                    {
                    	...
                    	if(_SmithWaterman_word_matrix[segment1tokenindexinmatrix - 1][segment2tokenindexinmatrix - 1] == 0 && _SmithWaterman_word_matrix[segment1tokenindexinmatrix - 1][segment2tokenindexinmatrix] == 0 && _SmithWaterman_word_matrix[segment1tokenindexinmatrix][segment2tokenindexinmatrix - 1] == 0)
                        {
                    		...
                    		//_editpenalizer.setWords(_segment1tokenlist[segment1tokenindexinmatrix - 1], _segment2tokenlist[segment2tokenindexinmatrix - 1]);
                    		_gappenalty = _editpenalizer.getSemanticMatchValue();
                    		_SmithWaterman_word_matrix[segment1tokenindexinmatrix][segment2tokenindexinmatrix] = _gappenalty;
                    		
                    		// The "approximately complete" semantic-match boundary case
                    		if(_SmithWaterman_word_matrix[segment1tokenindexinmatrix][segment2tokenindexinmatrix] >= 0.9)
                         	{
                         	...
                         	else
                         	{
                         		...
                         		_globalmatrixmatchscore =  _globalmatrixmatchscore + _SmithWaterman_word_matrix[segment1tokenindexinmatrix][segment2tokenindexinmatrix];
                        		_upperboundcumulativescore = _upperboundcumulativescore + _SmithWaterman_word_matrix[segment1tokenindexinmatrix][segment2tokenindexinmatrix];
                        		//_upperboundcumulativescore++;
                         		
                         		
                         		//_gappenalty = _editpenalizer.getSemanticGapValue("DEFAULT");
                         		//_globalmatrixmatchscore = _globalmatrixmatchscore + _gappenalty;
                         		//System.out.println("The Gap penalty for boundary is: " + _gappenalty + " and the CUMULATIVE SCORE (BOUNDRY) is: " + _globalmatrixmatchscore);
                     			
                         		//_globalmatrixmatchscore = _globalmatrixmatchscore + _SmithWatermanmatrix[segment1tokenindexinmatrix][segment2tokenindexinmatrix];
                    			//_upperboundcumulativescore = _upperboundcumulativescore + _SmithWatermanmatrix[segment1tokenindexinmatrix][segment2tokenindexinmatrix];
                         		
                         		//_fullmatchcount++;
                         		/*
                         		System.out.println("The CUMULATIVE SCORE for MISMATCH (BOUNDARY) so far is: " + _globalmatrixmatchscore);
                            	System.out.println("Final SmithWaterman value of " + segment1tokenindexinmatrix +" and " + segment2tokenindexinmatrix+ " is "+ _SmithWatermanmatrix[segment1tokenindexinmatrix][segment2tokenindexinmatrix]);
                            	System.out.println("The UB CUMULATIVE SCORE for MISMATCH (BOUNDARY) so far: " + _upperboundcumulativescore);
                            	System.out.println("Difference between cumulative and match count is: " + (_globalmatrixmatchscore - _upperboundcumulativescore));
                            	*/
                         	}
                        }
                    	...
                    	else
                    	{
                        	...
                        	if(_gappenalty >= - 0.10 && _SmithWaterman_word_matrix[segment1tokenindexinmatrix][segment2tokenindexinmatrix] < 1)
                            {
                        		if(_gappenalty < 0)
                        		{
                        			_gappenalty = 0;
                        		...
                        		_SmithWaterman_word_matrix[segment1tokenindexinmatrix][segment2tokenindexinmatrix] = Math.max(0, Math.max(_SmithWaterman_word_matrix[segment1tokenindexinmatrix - 1][segment2tokenindexinmatrix - 1] + _gappenalty, Math.max(_SmithWaterman_word_matrix[segment1tokenindexinmatrix - 1][segment2tokenindexinmatrix] + _editpenalizer.getSemanticGapValue("INS"), _SmithWaterman_word_matrix[segment1tokenindexinmatrix][segment2tokenindexinmatrix - 1] + _editpenalizer.getSemanticGapValue("DEL"))));
                        		
                        		_globalmatrixmatchscore = _globalmatrixmatchscore + _SmithWaterman_word_matrix[segment1tokenindexinmatrix][segment2tokenindexinmatrix];	
                        		
                        		//_globalmatrixmatchscore = (_gappenalty * _SmithWatermanmatrix[segment1tokenindexinmatrix][segment2tokenindexinmatrix]) + _globalmatrixmatchscore - _gappenalty;
                          	   	_upperboundcumulativescore = _SmithWaterman_word_matrix[segment1tokenindexinmatrix][segment2tokenindexinmatrix] + _upperboundcumulativescore;
                        		//_upperboundcumulativescore++;
                          	   	_editpenalizer.incrementLValue();
                          	   	_lastbestglobalmatrixmatchscore = _globalmatrixmatchscore; 
                            ...
                            {
                            	_SmithWaterman_word_matrix[segment1tokenindexinmatrix][segment2tokenindexinmatrix] = Math.max(0, _SmithWaterman_word_matrix[segment1tokenindexinmatrix - 1][segment2tokenindexinmatrix - 1] + _gappenalty);
                            ...
                            {
                            	_SmithWaterman_word_matrix[segment1tokenindexinmatrix][segment2tokenindexinmatrix] = Math.max(0, Math.max(_SmithWaterman_word_matrix[segment1tokenindexinmatrix - 1][segment2tokenindexinmatrix - 1] + _gappenalty, Math.max(_SmithWaterman_word_matrix[segment1tokenindexinmatrix - 1][segment2tokenindexinmatrix] + _editpenalizer.getSemanticGapValue("INS"), _SmithWaterman_word_matrix[segment1tokenindexinmatrix][segment2tokenindexinmatrix - 1] + _editpenalizer.getSemanticGapValue("DEL"))));
                        		double temp = _globalmatrixmatchscore + _SmithWaterman_word_matrix[segment1tokenindexinmatrix][segment2tokenindexinmatrix];
                            	                        		
                        		if(temp > _globalmatrixmatchscore)
                    			{
                    				_globalmatrixmatchscore = temp;
                    				_upperboundcumulativescore++;
                        		...
                        		{
                        			_globalmatrixmatchscore = (- _gappenalty * _globalmatrixmatchscore);
                        		}
                        		if(_lastbestglobalmatrixmatchscore < _globalmatrixmatchscore)
                        		{
                        			_lastbestglobalmatrixmatchscore = _globalmatrixmatchscore;
                        		}
                        		//_upperboundcumulativescore = _upperboundcumulativescore + _SmithWatermanmatrix[segment1tokenindexinmatrix][segment2tokenindexinmatrix];
                        		//_upperboundcumulativescore++;
                        		/*
                        		System.out.println("The CUMULATIVE SCORE for POOR SEMANTIC-MATCH so far is: " + _globalmatrixmatchscore);	
                        		System.out.println("Final SmithWaterman value of " + segment1tokenindexinmatrix +" and " + segment2tokenindexinmatrix+ " is "+ _SmithWatermanmatrix[segment1tokenindexinmatrix][segment2tokenindexinmatrix]);
                            	System.out.println("The UB CUMULATIVE SCORE for POOR SEMANTIC-MATCH so far is: " + _upperboundcumulativescore);
                            	System.out.println("Difference between cumulative and match count is: " + (_globalmatrixmatchscore - _upperboundcumulativescore));
                            	*/
                            	_editpenalizer.incrementLValue();
                            }
		...
		*/
		...
		/*
		System.out.println("Upper Bound of cumulative score: " + _upperboundcumulativescore);
		System.out.println("Final cumulative score: " + _globalmatrixmatchscore);
		System.out.println("Final last best cumulative score:" + _lastbestglobalmatrixmatchscore);
		*/
		...
		if(_upperboundcumulativescore >= 1)
		{
			...
			{
				double result = 1 + 0.1 * _editpenalizer.getOriginalGapValue("DEL", (Math.abs(_segment1tokenlist.length - _segment2tokenlist.length)));
			...
			else
			{
				...
				else
				{
					...
					//return Math.ceil(_globalmatrixmatchscore)/Math.ceil(_upperboundcumulativescore);
				}
			}
		}
		else
		{
			...
			//System.out.println("The final similarity value is: " + _globalmatrixmatchscore);
